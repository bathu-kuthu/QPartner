...
export default function ActiveFoodDeliveryScreen() {
    ...
    useEffect(() => {
        const handler = BackHandler.addEventListener('hardwareBackPress', () => {
            Alert.alert(
                ...
                'You cannot exit during an active food delivery. Please complete the delivery.',
                [{ text: 'OK' }]
            );
            return true;
        });
    ...
    const loadOrder = async () => {
        ...
        setNetError(false);
        try {
            const data = await FoodDriverService.getOrderById(id);
            if (data) {
                setOrder(data);
    ...
    useEffect(() => {
        ...
        channelRef.current = FoodDriverService.subscribeToOrder(id, (updated) => {
            setOrder(updated);
            if (updated.status === 'delivered') {
                setShowCompletionModal(true);
            } else if (updated.status === 'cancelled') {
                Alert.alert('Order Cancelled', 'This order was cancelled by the customer or store.', [
    ...
    const handlePickupFood = async () => {
        if (!order || updating) return;
        setUpdating(true);
        try {
            const updated = await FoodDriverService.updateOrderStatus(order.id, 'out_for_delivery');
            setOrder(updated);
            NotificationService.notifyRideStatus('Food Picked Up! 🍜', 'Head to the customer address for delivery.', order.id);
        } catch (e: any) {
    ...
    const handleConfirmDelivery = () => {
        ...
        Alert.alert(
            ...
            'Have you handed over the complete food package to the customer?',
            [
    ...
    if (loading) {
        return (
            <View style={[styles.center, { paddingTop: insets.top }]}>
                ...
                <Text style={styles.loadingText}>Loading Food Delivery...</Text>
            </View>
        );
    }
        ...
        <View style={[styles.container, { paddingTop: insets.top }]}>
            ...
            <View style={styles.header}>
                <View style={styles.stageIndicator}>
                    <View style={[styles.stageDot, { backgroundColor: '#FF6B35' }]} />
                    <Text style={styles.stageTitle}>
                        {isOutForDelivery ? 'ON THE WAY TO CUSTOMER' : 'HEAD TO RESTAURANT FOR PICKUP'}
                    </Text>
                </View>
            ...
            <ScrollView style={styles.scroll} showsVerticalScrollIndicator={false}>
                {/* ── STAGE 1: Restaurant Pickup Card ───────────────────────────────── */}
                <View style={[styles.card, !isOutForDelivery && styles.cardActive]}>
                    <View style={styles.cardHeader}>
                        <View style={[styles.cardIconBox, { backgroundColor: '#FFEDD5' }]}>
                            <MaterialCommunityIcons name="silverware-fork-knife" size={20} color="#FF6B35" />
                        </View>
                        <View style={{ flex: 1 }}>
                            <Text style={styles.cardTag}>1. PICKUP RESTAURANT</Text>
                            <Text style={styles.cardTitle}>{order.store?.name || 'Restaurant'}</Text>
                        </View>
                        <TouchableOpacity
                            style={styles.callCircleBtn}
                            onPress={() => makePhoneCall(order.store?.phone)}
                        >
                            <Feather name="phone" size={18} color="#FF6B35" />
                        </TouchableOpacity>
                    </View>

                    <Text style={styles.addressLine}>{order.store?.address || 'Restaurant address confirmed'}</Text>
                    ...
                    <View style={styles.checklistSection}>
                        <Text style={styles.checklistTitle}>Order Items Checklist ({items.length})</Text>
                        {items.map((item, idx) => {
                        ...
                        >
                            {updating ? (
                                ...
                                <>
                                    ...
                                    <Text style={styles.primaryActionBtnText}>CONFIRM ORDER PICKUP</Text>
                                </>
                            )}
                        </TouchableOpacity>
                    )}
            ...
            <Modal visible={showCompletionModal} animationType="fade" transparent>
                <View style={styles.modalOverlay}>
                    <View style={styles.modalContent}>
                        ...
                        <Text style={styles.modalSubtitle}>Food package delivered successfully to the customer.</Text>
...
const styles = StyleSheet.create({
    ...
    retryBtn: { marginTop: 16, backgroundColor: '#FF6B35', paddingHorizontal: 20, paddingVertical: 10, borderRadius: 12 },
    ...
    cardActive: { opacity: 1, borderColor: '#FF6B35' },
    ...
    primaryActionBtn: { flexDirection: 'row', justifyContent: 'center', alignItems: 'center', backgroundColor: '#FF6B35', height: 52, borderRadius: 14, marginTop: 8 },
    ...
    modalBtn: { backgroundColor: '#FF6B35', height: 48, borderRadius: 14, justifyContent: 'center', alignItems: 'center', width: '100%' },
